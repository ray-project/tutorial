...
def test_chain_env_spaces(chain_env_cls):
    ...
    assert test_env.action_space.contains(1), "Action Space does not contain 1."
    assert not test_env.observation_space.contains(6), "Observation Space is only [0..5]"
    assert test_env.observation_space.contains(5), "Observation Space is only [0..5]"
...
def test_chain_env_reward(chain_env_cls):
    ...
    assert total_reward == 0, "Expected {} reward; got {}".format(0, total_reward)
    for i in range(3):
